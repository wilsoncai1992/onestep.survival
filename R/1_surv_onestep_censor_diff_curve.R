...
#'
#' dW <- rep(1, nrow(dat))
#' onestepfit <- surv.one.step.differnece(dat = dat,
#'                                        dW = dW,
#'                                        verbose = FALSE,
#'                                        epsilon.step = 1e-3,
#'                                        max.iter = 1e3)
...
#' @import abind
#' @import SuperLearner
surv.one.step.differnece <- function(dat,
                          dW = rep(1, nrow(dat)),
    ...
    h.hat.t_1 <- haz_SL_wrapper(dat = dat, T.uniq = T.uniq, ht.SL.Lib = ht.SL.Lib)
    h.hat.t_0 <- haz_SL_wrapper(dat = dat0, T.uniq = T.uniq, ht.SL.Lib = ht.SL.Lib)
    ...
    G.hat.t_1 <- censor_SL_wrapper(dat = dat, T.uniq = T.uniq,
                                 Delta.SL.Lib = Delta.SL.Lib)
    G.hat.t_0 <- censor_SL_wrapper(dat = dat0, T.uniq = T.uniq,
                                 Delta.SL.Lib = Delta.SL.Lib)
    # cutoff <- 0.1
    cutoff <- 0.05
    ...
    # while ((stopping.criteria >= tol) & (iter.count <= max.iter) & ((stopping.prev - stopping.criteria) >= max(-tol, -1e-5))) { #WILSON: TEMPORARY
        ...
        # ------------------------------------------------------------------------
        # vectorized
        # update.mat <- compute.update.difference(D1.t.func.prev = D1.t,
    ...
    variables <- list(T.uniq = T.uniq, Psin.current = Psin.current, D1.A1.t = D1.A1.t, D1.t = D1.t, Pn.D1.t = Pn.D1.t, sup_norm_EIC = sup_norm_EIC)
    ...
    initial_fit_1 <- list(h.hat.t_1 = h.hat.t_1, Qn.A1.t_1 = Qn.A1.t_1, qn.A1.t_1 = qn.A1.t_1, G.hat.t_1 = G.hat.t_1)
    initial_fit_0 <- list(h.hat.t_0 = h.hat.t_0, Qn.A1.t_0 = Qn.A1.t_0, qn.A1.t_0 = qn.A1.t_0, G.hat.t_0 = G.hat.t_0)
